...
__global__ void gemm_memory_coalesce(int m, int n, int k, float alpha, float *A, float *B, float beta, float *C) {
...
void run_kernel(int m, int n, int k, float alpha, float *A, float *B, float beta, float *C) {
    ...
    gemm_memory_coalesce<<<grid_dim, dim3(32, 32)>>>(m, n, k, alpha, A, B, beta, C);
}
